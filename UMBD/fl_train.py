...
parser.add_argument('--arch', '-a', metavar='ARCH', default='3',
                    # choices=model_names,
...
def base_model():
    return MLP(48, 1, int(args.arch)).cuda()
...
def main():
    ...
    train_dataset = RetinopathyDatasetTrain(csv_file='../datasets/bank/data_train.npy', transform=transform_train, test=args.celoss)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=4)

    val_dataset = RetinopathyDatasetTrain(csv_file='../datasets/bank/data_test.npy', transform=transform_test, test=True)
    ...
    for i in range(args.clients):
        c_train_dataset = RetinopathyDatasetTrain(csv_file='../datasets/bank/data_train.npy', transform=transform_train, split=(i, args.clients), test=args.celoss)
